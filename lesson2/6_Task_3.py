...
class Position(Worker):
    ...
    def get_full_name(self):
        print(f'Employee`s full name - {self.name} {self.surname}')

    def get_total_income(self):
        income = self._income.get('wage') + self._income.get('bonus')
        print(f'Total income of {self.name} {self.surname} is {income}$')


emp_1 = Position('Tom', 'Sawyer', 'Traveler', 20000, 300)
emp_1.get_full_name()
